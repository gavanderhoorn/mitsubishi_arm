...
class JointServer(DatagramProtocol):
    ...
    def datagramReceived(self, datagram, address):
        ...
        self._mxt_cmd.dat.jnt.j4 = self._last_jnt[3]
        self._mxt_cmd.dat.jnt.j5 = self._last_jnt[4]
        self._mxt_cmd.dat.jnt.j6 = self._last_jnt[5]

        self.transport.write(ffi.buffer(self._mxt_cmd), address)
        #print "Finished sending reply."

class ControlServer(Protocol):
    def dataReceived(self, data):
        if data == "STORMLAB":
            self.transport.write("1\r")
        else:
            self.transport.write("-1\r")
        print "Finished sending reply."
...
f = Factory()
f.protocol = ControlServer

reactor.listenTCP(10003, f)
reactor.listenUDP(10000, JointServer())
